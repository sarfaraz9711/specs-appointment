...
const updateLogo = ()=>{
  if(router.asPath!='/' && (router.asPath=='/rc-sports/home' || sessionStorage.getItem("parentCategorySlug")=='rc-sports')){
    setLogoAction('furo')
  }else{
  ...
  // }
    ...
    window.addEventListener('click',function(e){
      let _except = ['ps-form--quick-search','list-inline-item','','anticon anticon-search bluecolor'];
...
const [getUserDetails, setUserDetails]=useState({})
  ...
  return (
    ...
    <div className="action-button d-flex  ">
      ...
      {getUserLogin && <div className="profile-section">
            <button
...
<script src="https://static-cdn.trackier.com/rtg/66e918d4e52a8164b41c58e4.js"></script>
    ...
    <div className="col-md-11">
      ...
      <ul className="list-inline helplineSec" style={{display:getSearchBarActive?'none':'inline-block'}}>
    <li className="list-inline-item marquee" style={{'color':'red'}}><div className="helpLine marqueeInner"><span>Sign Up And Get Add. 200 Off On Your First Order Above 1499 | Rs.100 Off On All Prepaid Orders Above 999 | Free Shipping On Order Above 999 |  Shop And Earn Loyalty Rewards | T&C Applied</span></div></li>

    </ul>
    <div className="search-hidden flex-grow-1" id="search-bar" style={{display:getSearchBarActive?'block':'none'}}>
    <SearchHeader />
		</div>
    <ul className="list-inline text-right">
    <li className="list-inline-item franchise-btn" style={{display:getSearchBarActive?'none':'inline-block'}}>
      <Link href="/franchise"><a className="btn btn-primary">Franchise</a></Link>
      </li>  
    <li className="list-inline-item"><a onClick={e=>searchBarToggle(getSearchBarActive)} href="javascript:void(0)" title="Click to Search"><i className={getSearchBarActive?'fa fa-times':'fa fa-search'} aria-hidden="true"></i> </a></li>
    <HeaderActions />
    </ul>
    </div>
    </div>
	...
	<div className="logo">
  {logoAction!='furo'?<Link href="/"><a title="Redchief"><img src="/static/img/red-chief-logo.png"/></a></Link>:
  <Link href="/"><a title="Redchief"><img src="/static/img/rc-sports-logo.png"/></a></Link>}
	</div>
	</div>
 ...
 <div>
   <button className="navbar-toggler" type="button" data-toggle="collapse" data-target="#collapsibleNavbar" onClick={()=>{
   dispatch(setResponseData(!mobileMenu));
   }}>
     {!mobileMenu && <span className="navbar-toggler-icon"></span>}
     {mobileMenu && <img src="/static/img/close-icon.png" />}
   </button>
   <div className={`collapse navbar-collapse ${mobileMenu?"mobile-menu-active":"mobile-menu-deactive"}`} id="collapsibleNavbar">
