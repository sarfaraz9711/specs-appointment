...
import {
  verifyCouponData,
  verifyCouponDataCN,
} from "../../../api/freeProduct/freeProduct";
import {
  getProfileData,
  addressListApiData,
  ApplyCouponApi,
  ...
  ingenicoCheckout,
  getLocationListApi,
  getCODAvailableOnPincode,
  getDeliveryAvailableOnPincode,
  pinCodeApiForDelivery,
  sendOtpToUser,
  otpVerificationApi,
  UserRegister,
  checkRegistredUser,
  saveIngenicoRequestPayload,
  savePaytmRequestPayload,
  cnStatusUpdate,
  syncInventoryBySku,
  checkSkuStatus,
} from "../../../api";
...
export async function getRecentRemovedProducts(setRecentRemovedProducts) {
  const result = await APIServices.getAll(
    `product-store/get-products-removed-from-cart`
  );

  if (result && result.data && result.data.data) {
    const pids = result.data.data;
    const products = await APIServices.getAll(
      `list/custom-product-list?productIds=${pids}`
    );
...
function Checkout() {
  ...
  const [cartValueDiscountFlagPercentage, setCartValueDiscountFlagPercentage] =
    useState(false);
  const [getDiscountPercentageValue, setDiscountPercentageValue] = useState(0);
  const [
    cartValueDiscountedAmountPercentage,
    setCartValueDiscountedAmountPercentage,
  ] = useState(0);
  const [congratsMsgToAvailFreeItems, setCongratsMsgToAvailFreeItems] =
    useState(false);
  ...
  const [availedCouponBasedPromoInfo, setAvailedCouponBasedPromoInfo] =
    useState({});
  const [availedCouponBasedPromoInfoCN, setAvailedCouponBasedPromoInfoCN] =
    useState({});
  ...
  const [isOrderConfirmationBoxActive, setOrderConfirmationBoxActive] =
    useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [itemRemoveProduct, setItemRemoveProduct] = useState({});
  const [shippingCharges, setShippingCharges] = useState(0);
  const [getTotalCartValueWithoutShipping, setTotalCartValueWithoutShipping] =
    useState(0);
  ...
  const carouselSetting1 = {
    ...
    nextArrow: (
      <NextArrow
        ...
        sendActionValue={sendActionValue}
      />
    ),
    prevArrow: <PrevArrow />,
    autoplay: false,
  ...
  const couponSubmit = () => {
    if (couponInput == "") {
      ...
      ApplyCouponApi(
        ...
        setCouponApplied,
        setAppliedName,
        totalData - promotionProductPrice,
        setcoupdata
      );
    }
  ...
  const addressSelect = async (address, totalCartAmount, index) => {
    ...
    cartLocale.forEach((item) => {
      ...
      if (
        beginCheckoutStorage.length == 0 ||
        !beginCheckoutStorage.some((itm) => itm.item_id == item.skuName) ||
        beginCheckoutStorage.length != cartLocale.length
      ) {
        beginCheckout = true;
      }
    });
  ...
  const handleData = async (pinCode, checkData) => {
    ...
    if (pinCode !== "") {
      await pinCodeApiForDelivery(pinCode).then(
        async (res) => {
          if (res.status == 200) {
            if (checkData == 1) {
    ...
    if (cartLocale) {
      cartLocale.forEach((product) => {
        ...
        cartFinal = {
          ...
          total: totalData - promotionProductPrice,
        };
  ...
  const totalInCart = async () => {
    ...
    for (var i = 0; i < len; i++) {
      ...
      let itemSum =
        locale[i].pricerefer !== ""
          ? locale[i].pricerefer
          : locale[i].initialPrice;

      itemSum = itemSum ? itemSum : locale[i].price;

      const actItemPrice =
        priceHelpFunc(
          locale[i].initialPrice ? locale[i].initialPrice : locale[i].price,
          locale[i].taxType,
          locale[i].taxValue,
          0
        ) * quantity;
      ...
      const totalSum =
        priceHelpFunc(itemSum, locale[i].taxType, locale[i].taxValue, 0) *
        quantity;
    ...
    if (allActivePromos) {
      if (allActivePromos != "No record Found") {
        const cartValuePercentageBasedOffers =
          allActivePromos.cartValueBased.percentageBased;
        const cartValueFreeItemBasedOffers =
          allActivePromos.cartValueBased.freeItemsBased;
        let availedOffers = [];
        cartValuePercentageBasedOffers.length > 0 &&
          cartValuePercentageBasedOffers.forEach((element) => {
            if (
              element.minCartTotalAmount <= sum &&
              sum < element.maxCartTotalAmount
            ) {
              const discountedAmount = Math.round(
                sum * (element.discountValue / 100)
              );
        ...
        let freeItemsIds = [];
        cartValueFreeItemBasedOffers.length > 0 &&
          cartValueFreeItemBasedOffers.forEach((element) => {
            element.freeProducts.forEach((item) => {
              let sumAfterFreeProduct;
              if (
                false &&
                locale.some((item1) => item1.productId == item.productId)
              ) {
              ...
              console.log(
                sumAfterFreeProduct,
                element.minCartTotalAmount,
                element.maxCartTotalAmount
              );
              if (
                sumAfterFreeProduct >= element.minCartTotalAmount &&
                sumAfterFreeProduct <= element.maxCartTotalAmount
              ) {
                freeItemsIds.push({
                  promoId: element.promoId,
        ...
        const offerProduct = locale.filter((item1) =>
          freeItemsIds.some((item2) => item2.productId == item1.productId)
        );
        ...
        if (offerProduct.length > 0) {
          offerProduct.forEach((element) => {
            ...
            appliedDiscountPerProd += priceHelpFunc(
              element.initialPrice ? element.initialPrice : element.price,
              element.taxType,
              element.taxValue,
              0
            );
            let findPromoId = freeItemsIds.filter(
              (item) => item.productId == element.productId
            );
            availedOffers.push({
              promoId: findPromoId[0].promoId,
              discountAmount: appliedDiscountPerProd,
              freeProductIds: element.productId,
        ...
        setAvailedCartBasedPromoInfo({
          promotionType: "CartValueBased",
          promoAvailedInfo: availedOffers,
        });
      }
      cartTotalCal = (
        sum -
        (appliedDiscountPerProd +
          appliedName +
          coupanPrice +
          coupanPriceCN +
          promotionProductPrice +
          getRedeemLoyaltyPoint +
          cartValueDiscountedAmountPercentage)
      ).toFixed(0);
    } else {
      cartTotalCal = (
  ...
  const prepaidOrderFunc = (totalCartValue) => {
    ...
    setTotalCartValue(
      totalCartValue + shippingChargesCal - prepaidOffPromotion
    );
    if (totalCartValue > 0) {
  ...
  const addressList = async (totalCartAmount) => {
    const resultData = await addressListApiData(
      setAddressData,
      setAddressLoader
    );
  ...
  const arrayCreate = () => {
    ...
    for (var i = 0; i < len; i++) {
      ...
      let skuName = "";
      locale[i].productvarientList &&
        locale[i].productvarientList.length > 0 &&
        locale[i].productvarientList.forEach((element) => {
  ...
  const addPromotionProduct = () => {
    ...
    let cartProductQuantity = countValuesByKey(
      cartItems,
      "productId",
      "quantity"
    );
    let promotionDetail = [];
    let promotionType1 = [
      "buy_x_and_get_x_free",
      "buy_x_and_get_x_percent",
      "buy_2x_and_get_2y_free",
    ];
    let promotionType2 = ["buy_2x_and_get_x_free", "buy_2x_and_get_x_percent"];
    let promotionType3 = [
      "buy_2x_and_get_2x_free",
      "buy_4x_and_get_x_amount",
      "buy_4x_and_get_x_percent",
    ];
    ...
    let promotionType6 = [
      "buy_4_any_and_get_x_amount",
      "buy_4_any_and_get_x_percent",
    ];
    let quantityCheckBuyXgetX = {
      1: 0,
      2: 1,
      3: 1,
      4: 2,
    ...
    cartItems &&
      ...
      cartItems.forEach((element) => {
        ...
        if (
          (setOfferProductData.length == 0 ||
            setOfferProductData.some(
              (e) => e.offerProductId != element.productId
            )) &&
          element.promotionProductYid != null
        ) {
          setOfferProductData.push({
            promotionId: element.promotionId,
            offerType: element.promotionType,
            offerProductId: element.productId,
            offerProductQuantity: offerProductQuantity,
            freeProductId: element.promotionProductYid,
            freeProductQuantity: freeProductQuantity,
            freeProductPrice: element.promotionFreeProductPrice,
          });
        }
      });
    ...
    const results =
      cartItems &&
      cartItems.length > 0 &&
      cartItems.filter(({ productId: id1 }) =>
        buyFourProductList.some(({ productId: id2 }) => {
          console.log(id2, id1);
          return id2 == id1;
        })
      );
    ...
    setOfferProductData.forEach((element) => {
      if (
        element.offerType == "buy_4_any_and_get_x_amount" &&
        buyFourProductList.length >= 4
      ) {
        promotionDetail.push({
          promoId: element.promotionId,
          discountAmount: element.freeProductPrice,
        });
        productDiscount +=
          results[0].promotionFreeProductPrice / buyFourProductList.length;
        productPromotionApply(promotionDetail, productDiscount);
        offerMessageCheck(element);
      } else if (
        (element.offerType == "buy_x_and_get_x_free" ||
          element.offerType == "buy_2x_and_get_x_free" ||
          element.offerType == "buy_2x_and_get_2x_free") &&
        cartProductQuantity[element.offerProductId] >=
          element.offerProductQuantity &&
        cartProductQuantity[element.freeProductId] >=
          element.freeProductQuantity
      ) {
        promotionDetail.push({
          promoId: element.promotionId,
          discountAmount: element.freeProductPrice,
        ...
        productDiscount +=
          element.freeProductPrice *
          quantity[cartProductQuantity[element.freeProductId]];
        productPromotionApply(promotionDetail, productDiscount);
        offerMessageCheck(element);
      } else if (
        (element.offerType == "buy_x_and_get_y_free" ||
          element.offerType == "buy_2x_and_get_y_free" ||
          element.offerType == "buy_2x_and_get_2y_free") &&
        cartProductQuantity[element.offerProductId] >=
          element.offerProductQuantity &&
        cartProductQuantity[element.freeProductId] >=
          element.freeProductQuantity
        ...
        quantityCheckBuyXgetY.forEach((item) => {
          if (cartProductQuantity[element.offerProductId] == item) {
            if (element.offerType == "buy_x_and_get_y_free") {
              ...
              const offerQuantity = (
                cartProductQuantity[element.offerProductId] / 2
              )
                .toString()
                .substring(0, 1);
              ...
              const offerQuantity = (
                ...
                .substring(0, 1);
              const freeQuantity = (
                cartProductQuantity[element.freeProductId] / 2
              )
                .toString()
                .substring(0, 1);
  ...
  const productPromotionApply = (promotionDetail, productDiscountPrice) => {
    ...
    if (flag) {
      const arrayUniqueByKey = [
        ...new Map(
          promotionDetail.map((item) => [item["promoId"], item])
        ).values(),
      ];
  ...
  const countValuesByKey = (arr, key, key2) =>
    arr &&
    arr.length > 0 &&
    arr.reduce((r, c) => {
  ...
  useEffect(() => {
    ...
    const filterResult =
      ...
      cartItems.filter(
        (data) =>
          new String(data.keywords)
            .toUpperCase()
            .indexOf(new String("ACCESSORIES").toUpperCase()) >= 0
      );
  ...
  const verifyOtp = async () => {
    ...
    if (result.status == 200) {
      ...
      productJson.forEach(async (element) => {
        await addToCartApi(
          element.productId,
          priceHelpFunc(element.price, element.taxType, element.taxValue, ""),
          element.quantity,
          "",
          "",
          setDummy,
  ...
  const proceedAfterOrderConfirmation = async () => {
    ...
    if (method == 8) {
      ...
      //setLoaderActive(true)
      let _dataOpt = await paytmCheckout(
        fname,
        lname,
        address,
        delivaryMobile,
          ...
          handler: {
            notifyMerchant: function (eventName, data) {
              ...
              if (
                ...
                data.message == "App closed from the header icon"
              ) {
      ...
      if (_dataOpt && _dataOpt.status == 200) {
        ...
        await saveIngenicoRequestPayload(
          configJson,
          _dataOpt.data.orderPrefixId
        );
  ...
  const handleClickToShowNetbankingOptions = (row) => {
    ...
    if (row && row.paymentMode === "NET_BANKING") {
      let allEnabledBanks =
        row &&
        row.payChannelOptions.length > 0 &&
        row.payChannelOptions.filter(
          (item) => item.isDisabled.status === "false"
        );
  ...
  const applyCoupon = async (val) => {
    ...
    if (coupanCodeText != "") {
      ...
      if (
        couponData.data &&
        !couponData.data.orderId &&
        (promotionApply || cartValueDiscountFlag)
      ) {
        setCounponErrorMsg(
          "Free product promotion applied. Please use coupon without promotion"
        );
        return false;
      }
      if (
        ...
        couponData.data.isActive == 1 &&
        couponData.data.couponPromotionType == "employeesOnly"
      ) {
        ...
        if (employeeProfile.data.status == 200) {
          ...
          cartItem.forEach((element) => {
            const productMrp = Math.round(
              +element.price + (element.price * element.taxValue) / 100
            );
            const discount =
              element.discount <= 25 ? 30 - element.discount : 10;
            console.log(productMrp, discount, element.quantity);
            coupanDiscount +=
              Math.round((productMrp * discount) / 100) * element.quantity;
          });
          ...
          setAvailedCouponBasedPromoInfo({
            ...
            promoAvailedInfo: [
              {
                promoId: couponData.data.couponId,
                discountAmount: coupanDiscount,
              },
            ],
          });
        } else {
          setCounponErrorMsg(
            "This coupon is only applicable for RSPL group employees"
          );
        }
      } else if (
        couponData &&
        ...
        if (couponData.data.couponType == 1) {
          coupanDiscount =
            (couponJson.couponValue * couponData.data.couponValue) / 100;
  ...
  const applyCouponCN = async () => {
    ...
    if (coupanCodeCN != "") {
      ...
      const couponDataCN = await verifyCouponDataCN(
        couponJsonCN.couponCodeCN,
        couponJsonCN.couponValueCN
      );
      if (couponDataCN?.status == 1) {
        ...
        setAvailedCouponBasedPromoInfoCN({
          ...
          promoAvailedInfo: [
            {
              promoId: couponDataCN.data.cnCode,
              discountAmount: couponDataCN.data.cnAmount,
            },
          ],
        });
        setCouponApplied(false);
    ...
    if (val == "Verify") {
      const redeemJson = {
        objClass: {
          ...
          customer_points:
            getBalanceLoyaltyPoint < 300 ? getBalanceLoyaltyPoint : "300",
    ...
    <div className="cart-container">
          ...
          <div className="custom-checkout-form-left">
            ...
            <div className="custom-checkout-form-contact">
              ...
              <div className="custom-field-row">
                <input
                  ...
                  onBlur={(e) => {
                    setNameFocus(false);
                    registerOnChange(e, 1);
                  }}
                  onChange={(e) => registerOnChange(e, 2)}
              ...
              <div className="custom-field-row">
                <input
                  ...
                  onBlur={(e) => {
                    setMailFocus(false), emailFocuOut();
                  }}
              ...
              {!authValue.isLoggedIn && (
                <div className="add-gst-number-container d-none">
                  <div className="add-gst-number-subcontainer">
                    <input
                      type="checkbox"
                ...
                <>
                  {authValue.isLoggedIn && (
                    <>
                        ...
                        addressData.map((address, index) => {
                          return (
                            <div className="address-container">
                              ...
                              >
                                {address.addressType === 1 ? "Home" : "Work"}
                              ...
                              <p className="address-paragraph">
                                {address.company}, {address && address.address1}
                                , {address.city}, {address.address2},{" "}
                                {address.state + " : " + address.postcode},{" "}
                                {address.phoneNo}
                                {address.phoneNoAlter ? "," : ""}{" "}
                                {address.phoneNoAlter}
                              </p>
                            </div>
                          );
                      ...
                      {delivaryTime1 && <span>{delivaryTime1}</span>}
              ...
              {totalCartValue != 0 && (
                <div className="payment-new-screen mobile-menu-div">
                          ...
                          paymentOption.map((pay) => (
                            ...
                            >
                              <DisplayImageWithS3PreSignedUrl
                                imageKey={
                                  pay.pluginAvatarPath + pay.pluginAvatar
                                }
                                resizeRequired="YES"
                                style={{ width: "100px" }}
                                alt=""
                              />{" "}
                              <span className="payment-resources">
                                {pay.resources}
                              </span>
                              {/* </div> */}
                            </Radio>
            ...
            <div className="recent-viewed-products">
              {recentRemovedProducts.length > 0 ? (
                <>
                  <h3 className="ml-3">
                    You showed intereset in the below products
                  </h3>
                  <Slider {...carouselSetting1} className="ps-carousel">
                    {recentRemovedProducts.map((item, i) => {
                      return (
                        <>
                          <div key={i}>
                            <Product
              ...
              <div>
                <div className="custom-checkout-product-card">
                  <table className="custom-checkout-card-table">
                        ...
                        <tr>
                          ...
                          <td className="custom-checkout-td-content">
                            <a
                              ...
                              style={{}}
                              onClick={(e) => itemRemoveConfirmation(product)}
                            >
                              x
                            </a>
                            ...
                            <div className="checkout-td-prooductname-subcontainer">
                              <p className="mt-0">
                                {product.productvarientList &&
                                  product.productvarientList.map((item) => (
                                    <>
                                ...
                                <p className="mt-0">
                                  Size: {product.variantName.split(",")[0]},
                                  Color: {product.variantName.split(",")[1]}
                                </p>
                              </p>
                              ...
                              <>
                                {product.promotionFlag == 1 && (
                                  <div className="custom-tier-proddet">
                                    <Link
                                      href={`/offers/get-free-items?productids=${encrptData(
                                        product.promotionProductYSlug
                                      )}`}
                                    >
                              ...
                              <>
                                {product.promotionFlag == 1 && (
                                  <div className="custom-tier-proddet">
                                    <Link
                                      href="/product/[pid]"
                                      as={`/product/${product.promotionProductYSlug}`}
                                    >
                                      <a className="color-link">
                                        {" "}
                                        {product.offerApplied &&
                      ...
                      <Modal.Body>
                        <div className="order-confirm-body">
                          ...
                          <div className="action-btns">
                            ...
                            <button className="proceed-btn" onClick={handleOk}>
                              Yes, Proceed
                            </button>
                          </div>
                        </div>
                    ...
                    <tr>
                      <td className="custom-td-total text-left">
                        {" "}
                        {t("total")}
                      </td>
                    ...
                    {cartValueDiscountFlag && (
                      <tr>
                        <td className="custom-td-total">
                          <p className="coupon-apply-text">
                            Free Product on Cart Discount
                          </p>
                        </td>
                        <td
                          className="custom-td-total"
                        ...
                        >
                          (-) {currency ? currency.symbol + " " : "₹ "}{" "}
                          {parseFloat(cartValueDiscountedAmount.toFixed(0))}{" "}
                        </td>
                      </tr>
                    )}
                    {cartValueDiscountFlagPercentage && (
                      <tr>
                        <td className="custom-td-total">
                          <p className="coupon-apply-text">
                            {getDiscountPercentageValue}% Discount on Cart Value
                          </p>
                        </td>
                        <td
                          className="custom-td-total"
                        ...
                        >
                          (-) {currency ? currency.symbol + " " : "₹ "}{" "}
                          {parseFloat(
                            cartValueDiscountedAmountPercentage.toFixed(2)
                          )}{" "}
                        </td>
                      </tr>
                    )}
                        ...
                        <td className="custom-td-total" colspan="2">
                          <div className="promotionalBox">
                            <div className="promotionalTitle">
                              ...
                              <div>
                                <h6>
                                  Coupon Code : <span>{coupanCode} </span>
                                </h6>
                              </div>
                              {newVal == "" ? (
                                <></>
                                ...
                                <div>
                                  <h6>
                                    Discount Value is : <span>{newVal} %</span>
                                  </h6>
                                </div>
                              )}
                            </div>
                        ...
                        <td className="custom-td-total" colSpan="2">
                          <div className="promotionalBox">
                            ...
                            <div className="promotionalAmount">
                              ...
                              <div className="close">
                                <a onClick={(e) => loyaltyPointApply(2, 0)}>
                                  x
                                </a>
                              </div>
                            </div>
                        ...
                        <td className="custom-td-total" colSpan="2">
                          <div className="promotionalBox">
                            <div className="promotionalTitle">
                              ...
                              <div>
                                <h6>Free shipping on order above 999</h6>
                              </div>
                            </div>
                            <div className="promotionalAmount">
                              <div style={{ textAlign: "right" }}>
                        ...
                        <td className="custom-td-total" colSpan="2">
                          <div className="promotionalBox">
                            <div className="promotionalTitle">
                              ...
                              <div>
                                <h6>100 Rs Off on prepaid order above 999</h6>
                              </div>
                            </div>
                            <div className="promotionalAmount">
                              <div style={{ textAlign: "right" }}>
                    ...
                    <tr>
                      <td colspan="2" class="custom-td-total">
                        {" "}
                ...
                {congratsMsgToAvailFreeItems && (
                  <div className="d-flex">
                    <div style={{ color: "green", marginRight: "4px" }}>
                      {" "}
                      🎉 Congratulations, you are eligble to get free products.
                      Add items in cart{" "}
                    </div>
                    <Link
                      href={`/offers/get-free-items?cartValue=${totalData}`}
                    >
                      <a style={{ color: "blue" }}>Free Products</a>
                    </Link>
                  </div>
                )}
                <div className="temp-data-hide loyality-point-card">
                  <h5> Loyalty Point </h5>
                  <div className="row justify-content-between align-item-center">
                    <div className="col-md-5">
                      Balance Loyalty Point: {getBalanceLoyaltyPoint}
                      {loyaltyErrorMsg && (
                        <p className="alert alert-danger">{loyaltyErrorMsg}</p>
                      )}
                    </div>
                    <div className="col-md-5">
                          ...
                          {!getPointApplied &&
                            ...
                            otpResendTime != 0 && (
                              <div className="otp-resend">
                                OTP will resend <span>{otpResendTime}</span>
                              </div>
                            )}
                          {invalidOtp && (
                            <p className="alert alert-danger">Invalid OTP</p>
                          )}
                        </>
                      )}
                    </div>
                    {getBalanceLoyaltyPoint > 0 && (
                        ...
                        <div>
                          {!getPointApplied && (
                            <input
                              ...
                              onClick={(e) => loyaltyPointApply(getBtnValue, 0)}
                              value={getBtnValue}
                            />
                          )}
                          {getPointApplied && (
                            <span className="alert alert-success">Applied</span>
                          )}
                        </div>
                        {otpResendBtn && getBtnValue == "Verify" && (
                          <input
                            type="button"
                            className="btn btn-primary btn-lg mt-2"
                            onClick={(e) => loyaltyPointApply("Apply", 60)}
                            value="Resend OTP"
                          />
                        )}
                      </div>
                ...
                {coupanPriceCN < 1 && (
                  <div className="temp-data-hide coupon-apply-card">
                    <h5> Apply Credit Note </h5>
                    <div className="row justify-content-between">
                      <div className="col-md-6">
                        <input
                          onChange={(e) => getCoupanValueCN(e)}
                          type="text"
                          className="form-control"
                          value={coupanCodeCN}
                ...
                {getCoupanData.length > 0 && coupanPrice < 1 && (
                  <div className="temp-data-hide coupon-apply-card">
                    ...
                    {getCoupanData.length > 0 &&
                      getCoupanData.map((item, i) => (
                        <>
                          {item.couponClass == "pending" && (
                            <div
                              key={i}
                              className="row justify-content-between coupon-list"
                            >
                              <div className="col-md-6 alert alert-success">
                                {item.coupanType == 2 ? " ₹" : ""}{" "}
                                {item.discount}
                                {item.coupanType == 1 ? "%" : ""} off coupon (
                                {item.coupanCode})<br />
                                Min Order value: {item.minAmount}
                              </div>
                              <div className="col-md-6 text-right ">
                                <input
                                  type="button"
                                  className="btn btn-primary btn-lg"
                                  onClick={(e) => applyCoupon(item.coupanCode)}
                                  value="Apply"
                                />
                              </div>
                            </div>
                ...
                <div className="delivery-card">
                  ...
                  <div className="row justify-content-between">
                    ...
                    <div className="col-md-6 text-right ">
                      <button
                        className="btn btn-primary btn-lg"
                        onClick={(e) => handleData(pincode, 2)}
                      >
                        Check
                      </button>
                    </div>
                  ...
                  <div className="row">
                    <p className="col-md-12" style={{ color: "#1976d2" }}>
                      {delivaryTime}
                    </p>
                ...
                {getDeliveryServiceable ? (
                  <>
                    {totalCartValue != 0 && (
                      <div className="payment-new-screen desktop-menu-div">
                        <p>{t("checkouts.SelectPaymentMode")}</p>
                        <div
                          className="payment-main-grid"
                          style={{
                  ...
                  <div className="alert alert-danger text-center">
                    Order Delivery is not available to your pincode
                  </div>
                )}
              </div>
            )}
              ...
              {payOps &&
                ...
                payOps.map((row) => {
                  return (
                    <li
                      className={
                        row.displayName == selectedPaymentOptionClass
                          ? "option-selected"
                          : ""
                      }
                      onClick={() => handleClickToShowNetbankingOptions(row)}
                      key={row.displayName}
                    >
                      {row.displayName}
                    </li>
            ...
            <main>
              ...
              {showNetbankingBanks && (
                <form
                  method="post"
                  type="redirect"
                  action={`${paytmSetting.hostUrl}/theia/api/v1/processTransaction?mid=${paytmSetting.merchantId}&orderId=${txnInitiationData.orderId}`}
                >
                  <input
                    type="hidden"
                    name="mid"
                    defaultValue={paytmSetting.merchantId}
                  />
                  <input
                    type="hidden"
                    name="orderId"
                    defaultValue={txnInitiationData.orderId}
                  />
                  <input
                    type="hidden"
                    name="txnToken"
                    defaultValue={txnInitiationData.txnToken}
                  />
                  <input
                    type="hidden"
                    name="paymentMode"
              ...
              {showCCDDForm && (
                ...
                >
                  ...
                  <input
                    type="hidden"
                    name="cardInfo"
                    value={`|${cardNumber}|${cardCvv}|${cardValidTillMonth}20${cardValidTillYear}`}
                  />
                  <input type="hidden" name="AUTH_MODE" defaultValue="otp" />
                  <div>
      ...
      <Modal className="order-confirm-box" show={isOrderConfirmationBoxActive}>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            <p>
              Dear Customer, Please confirm order. By clicking Confim button you
              will be redirected to Payment page.
            </p>
            <div className="action-btns">
              <button
                className="cancel-btn"
                onClick={closeOrderConfirmationBox}
              >
                Cancel
              </button>
              <button
                className="proceed-btn"
                onClick={proceedAfterOrderConfirmation}
              >
                Proceed
              </button>
            </div>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            <p className="alert alert-danger">
              Dear Customer,
              <br />
              Item SKU ({skuNameValidation}) you are trying to checkout is Out
              of Stock. Please try again later
            </p>
            <div className="action-btns">
              <button className="cancel-btn" onClick={closeSkuValidation}>
                Ok
              </button>
            </div>
          </div>
        ...
        <Modal.Body>
            ...
            <div className="otp-mobile-box">
              ...
              {showCustomCaptcha && (
                <CustomCaptcha
                  ...
                  setBrowserIdentifier={setBrowserIdentifier}
                  isCaptchaBlank={isCaptchaBlank}
                  reloadCaptchaOnError={reloadCaptchaOnError}
                />
              )}
              <div style={{ marginTop: "22px" }} className="action-btns">
                <button
                  className="proceed-btn"
                  onClick={(e) => captchaVerify(e)}
                >
                  Send OTP
                </button>
              </div>
          ...
          {!captchaVerifyOption && (
            <div className="order-confirm-body">
              ...
              <input
                type="text"
                onChange={(e) => otpValueLogin(e.target.value)}
                className="form-control mb-3"
                placeholder="Enter OTP"
              />
              {getOtpMessage && (
                <p className="alert alert-danger">{getOtpMessage}</p>
              )}
              <input type="checkbox" onClick={(e) => otpAgreement(e)} /> By
              continuing, I agree to the{" "}
              <Link href="/page-detail/terms-and-conditions">
                <a className="color-link">Terms of use</a>
              </Link>
              <div className="action-btns">
                <button
                  disabled={otpBtn || !otpLogin}
                  className="proceed-btn"
      ...
      <Modal className="order-confirm-box" show={alreadyUserError}>
        <Modal.Header>
          ...
          <button
            ...
            class="close"
            data-dismiss="modal"
            onClick={(e) => checkAlreadyUserError(e)}
            aria-label="Close"
          >
            <span aria-hidden="true">&times;</span>
          </button>
        ...
        <Modal.Body>
          <h5 className="alert alert-danger mb-0 pt-3 pb-3">
            Please login for checkout, email or mobile already exist in our
            records
          </h5>
        </Modal.Body>
      </Modal>
      ...
      <Modal className="order-confirm-box" show={loginSuccessPopup}>
        <Modal.Header>
          ...
          <button
            ...
            class="close"
            data-dismiss="modal"
            onClick={(e) => checkAlreadyUserSuccess(e)}
            aria-label="Close"
          >
            <span aria-hidden="true">&times;</span>
          </button>
      ...
      {isLoaderActive && (
        <div className="loader-page">
          ...
          {!isPageLoader && (
            <div>
              We are redirecting to you on payment gateway
              <br />
              Do not back and refresh the page
            </div>
          )}
        </div>
      )}
