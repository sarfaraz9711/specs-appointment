...
export default function AppointmentBooking() {
    ...
    if (result.status == 200) {
      ...
      setBookedSlots({
        [calendarData.bookedDate]:
          calendarData.bookedSlot && calendarData.bookedSlot.split(","),
      });
  ...
  const buildSlots = (startTime, endTime) => {
    ...
    while (start < end) {
      ...
      const label = `${String(hour12).padStart(2, "0")}:${
        m === 0 ? "00" : m
      } ${ampm}`;
      slots.push(label);
  ...
  const updateAppointmentFun = async (val) => {
    ...
    if (val == 1) {
      ...
      setMessage({
        show: true,
        message: `✅ Your appointment has been updated to ${displayDate} at ${selectedSlot}`,
        class: "alert alert-success",
      });
    } else {
      json.isActive = 0;
      ...
      <div className="appointment-container">
        {getNextAppointment && (
          <div className="appointment-alert">
            ...
            {moment(getNextAppointment?.appointmentDate).format("DD-MM-YYYY")}{" "}
            at {getNextAppointment?.appointmentTime}.
            <input
              type="button"
              ...
              {allSlots.map((slot) => {
                ...
                return (
                  <button
                    ...
                    className={`slot-btn ${
                      isTaken || isPastSlot(slot, selectedDate)
                        ? "slot-booked"
                        : ""
        ...
        {getSlotAvailableMsg && (
          <div className="alert alert-warning d-flex justify-content-between mt-3">
            You have already an appointment on this date{" "}
            {moment(getSlotAavailableData.appointmentDate).format("DD-MM-YYYY")}{" "}
            at {getSlotAavailableData.appointmentTime}{" "}
            <input
              type="button"
        ...
        {/* Status message */}
        {message.show && (
          <p className={`${message.class} mt-5`}>{message.message}</p>
        )}
      </div>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            <p>
              ...
              {getSlotAavailableData.appointmentTime}. Do you want to reschedule
              it to {selectedSlot}?
            </p>
            <div className="action-btns">
              ...
              <button
                ...
                onClick={() => updateAppointmentFun(1)}
              >
                Update
              </button>
            </div>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            ...
            <div className="action-btns">
              ...
              <button
                className="proceed-btn"
                onClick={() => updateAppointmentFun(2)}
              >
                Submit
              </button>
            </div>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            <p>
              ...
              <div className="p-relative">
                <div>
                  ...
                  {sentOtp && (
                    <div className="otp-color">OTP Sent on above number</div>
                  )}
                </div>
                {sentOtp && (
                  <i
              ...
              {sentOtp && (
                <div className="p-relative">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && getOtp.length > 0) {
                        VerifyOtp();
                      }
                    }}
                  />
                  {sentOtp && (
                    <input
                      className={`resend-otp ${
                        getResendTimeActive ? "" : "btn btn-primary"
                      }`}
                      disabled={getResendTimeActive}
                      type="button"
                      value={`OTP Resend${
                        getResendTimeActive ? `(${getOtpResendTime})` : ""
                      }`}
                    />
                  )}
                </div>
            ...
            <div className="action-btns">
              ...
              {!sentOtp && (
                <button className="proceed-btn" onClick={() => sendOtp()}>
                  Send OTP
                </button>
              )}
              {sentOtp && (
                <button className="proceed-btn" onClick={() => VerifyOtp()}>
                  Verify OTP
                </button>
              )}
            </div>
        ...
        <Modal.Body>
          <div className="order-confirm-body">
            <div className="alert alert-success text-center">
              OTP verified successfully for mobile number {getMobileNo}
            </div>
            <div className="row">
