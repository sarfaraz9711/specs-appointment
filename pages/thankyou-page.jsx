...
const ThankYouPage = () => {
  ...
  const queryData = JSON.parse(
    Buffer.from(router.query.queryData, "base64").toString()
  );
  ...
  useEffect(() => {
    if (queryData) {
      console.log(
        queryData,
        "Neeraj Product View payttfffffffffffffffffffffffffffffff"
      );
      let fbItems = [];
      const items =
        queryData &&
        queryData.orderData.productDetail.map((item) => {
          fbItems.push({
            currency: "INR",
      ...
      let payment = {
        ...
        ecommerce: {
          ...
          items: items,
          payment_type:
            queryData.orderData.paymentMethod == 2 ? "COD" : "Online",
        },
      };
...
'transaction_id': '${queryData.orderPrefixId}'
      ...
      {/* <FooterFullwidth /> */}
      <iframe
        src={`https://tracking.icubeswire.co/aff_a?offer_id=482&order_id=${queryData.orderData.orderPrefixId}&sale_amount=${queryData.orderData.total}`}
        width="1"
        height="1"
        id="pixelcodeurl"
      ></iframe>
      <iframe
        src={`https://mobilogi.gotrackier.com/pixel?adid=66e91685b8d1f801de70c95d&txn_id=${queryData.orderData.orderPrefixId}&sale_amount=${queryData.orderData.total}`}
        frameborder="0"
        width="1"
        height="1"
      ></iframe>
